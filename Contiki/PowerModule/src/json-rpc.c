...
void decode_json_rpc(char *json_string, struct tuple *tup)
{
	...
	for(index = 0; index < length; index++)
	{
		...
		{
			strncpy(tup->ip, json_string+jsmn_tokens[index+1].start, (jsmn_tokens[index+1].end - jsmn_tokens[index+1].start));
			index++;
		}
...
void get_ip_addr_from_json(char *ipaddr, char *json_string){
	...
	for(index = 0; index < length; index++)
	{
		...
		{
		strncpy(ipaddr, json_string+jsmn_tokens[index+1].start, (jsmn_tokens[index+1].end - jsmn_tokens[index+1].start));
		index++;
		}
	}
}
...
void get_id_from_json(char *id, char *json_string){
	...
	for(index = 0; index < length; index++)
	{
		...
		{
		strncpy(id, json_string+jsmn_tokens[index+1].start, (jsmn_tokens[index+1].end - jsmn_tokens[index+1].start));
		index++;
		}
	}
}
